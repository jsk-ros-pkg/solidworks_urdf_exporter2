    ...
    for rec in ordered_links:
        ...
        if axis_direction is None:
            ...
            if axis_xyz is not None:
                try:
                    axis_direction = np.asarray(axis_xyz, float)
                except Exception:
                    axis_direction = None
            if axis_direction is None or float(np.linalg.norm(axis_direction)) < _EPS:
                if joint_type in ("revolute", "continuous", "prismatic"):
                    _warn("payload step 3 failed: movable joint "
                          f"{joint_name!r} has no usable axis data")
                    return None
            else:
                axis_direction = axis_direction / float(np.linalg.norm(axis_direction))
